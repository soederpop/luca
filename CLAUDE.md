...
> **Note for agents:** references to `.claude/skills/luca-framework/SKILL.md` apply to *bootstrapped consumer projects* (created via `luca bootstrap`). In this framework repo that path does not exist — the source of that skill lives at [docs/bootstrap/SKILL.md](docs/bootstrap/SKILL.md).
