...
async function bootstrap(options: z.infer<typeof argsSchema>, context: ContainerContext) {
	...
	ui.print('    luca serve --open     — view an intro to luca in your browser')
...
// A guided tour of the env vars luca reads. Everything stays commented out on
// purpose: the CLI loads .env into process.env, and a live placeholder value
// (e.g. OPENAI_API_KEY=sk-...) would make the default-provider chain pick that
// provider and then fail auth, instead of falling back to the local model.
// Uncomment only the lines you actually set.
export const DEFAULT_ENV = [
	'# ─────────────────────────────────────────────────────────────────────',
	'# luca .env — the CLI loads this automatically before your luca.cli.ts runs.',
	'# Everything below is commented out; uncomment only what you set.',
	'# ─────────────────────────────────────────────────────────────────────',
	'',
	'# ── Model provider (chat / assistants) ──────────────────────────────',
	'# Pin the default provider. Omit to auto-detect: openai (if a key is set),',
	'# then local (if `luca setup` installed a model), then anthropic.',
	'# LUCA_DEFAULT_PROVIDER=local',
	'',
	'# Use OpenAI (or any OpenAI-compatible endpoint) as the model provider.',
	'# OPENAI_API_KEY=sk-...',
	'# Point the OpenAI client at a compatible server (Ollama, vLLM, LiteLLM).',
	'# OPENAI_BASE_URL=http://localhost:11434/v1',
	'# Default chat model when none is passed (default: gpt-5.4-mini).',
	'# OPENAI_DEFAULT_MODEL=gpt-5.4-mini',
	'',
	'# Use Anthropic as the model provider.',
	'# ANTHROPIC_API_KEY=sk-ant-...',
	'',
	'# ── Local inference (llama-server, via `luca setup`) ────────────────',
	'# Base URL + auth for the local chat endpoint (claude-code local mode).',
	'# LOCAL_CHAT_ENDPOINT=http://localhost:1234',
	'# LOCAL_CHAT_AUTH_TOKEN=sk-...',
	'# Model id used for local coding tasks (default: qwen/qwen3.6-27b).',
	'# LOCAL_CODER_MODEL=qwen/qwen3.6-27b',
	'# Override the pinned llama.cpp release tag / platform asset (advanced).',
	'# LUCA_LLAMA_RELEASE=b1234',
	'# LUCA_LLAMA_ASSET=llama-b1234-bin-macos-arm64.tar.gz',
	'',
	'# ── Semantic search / embeddings (`luca describe --query`) ──────────',
	'# Where `luca describe --query` generates embeddings: local (default, via',
	'# `luca setup --local-embeddings`) or openai (any OpenAI-compatible endpoint).',
	'# LUCA_EMBEDDING_PROVIDER=openai',
	'# For the openai provider, these override host / key / model just for',
	'# embeddings (else fall back to OPENAI_BASE_URL / OPENAI_API_KEY).',
	'# LUCA_EMBEDDING_BASE_URL=http://localhost:11434/v1',
	'# LUCA_EMBEDDING_API_KEY=sk-...',
	'# LUCA_EMBEDDING_MODEL=text-embedding-3-small',
	'',
	'# ── Plugins & command discovery ─────────────────────────────────────',
	'# Comma-separated plugins to auto-load (names under ~/.luca/plugins, or paths).',
	'# LUCA_PLUGINS=my-plugin,../another-plugin',
	'# Limit helper discovery: disable | no-local | no-home | commands-only.',
	'# LUCA_COMMAND_DISCOVERY=no-home',
	'# Override where luca stores its home dir (binaries, models, indexes).',
	'# LUCA_HOME=~/.luca',
	'# Silence the skills-library warning when no skills are present.',
	'# LUCA_SKILLS_NO_WARN=1',
	'',
	'# ── MCP server compatibility ────────────────────────────────────────',
	'# Set to "codex" to serve MCP in OpenAI Codex-compatible mode.',
	'# MCP_HTTP_COMPAT=codex',
	'# stdio transport compatibility: codex | auto | standard.',
	'# MCP_STDIO_COMPAT=auto',
	'',
	'# ── Misc ────────────────────────────────────────────────────────────',
	'# Show full stack traces on errors (same as passing --verbose).',
	'# DEBUG=1',
	'# Connection string for the `postgres` feature.',
	'# DATABASE_URL=postgres://user:pass@localhost:5432/db',
	'',
	'# ── Optional third-party integrations (only if you use these helpers) ─',
	'# GITHUB_TOKEN=ghp_...',
	'# ELEVENLABS_API_KEY=...',
	'# RUNPOD_API_KEY=...',
	'# TELEGRAM_BOT_TOKEN=...',
