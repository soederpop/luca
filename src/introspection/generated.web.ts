...
setBuildTimeData('features.vault', {
  ...
  "methods": {
    ...
    "unlock": {
      "description": "Derives this vault's key from a passphrase (PBKDF2-SHA256, 600,000 iterations, salted by vault ID). The same passphrase and vault ID give the same key on any device. The key is kept in memory only; it replaces any device key for this instance until `lock()`.",
      "parameters": {
        "passphrase": {
          "type": "string",
      ...
      "examples": [
        {
          "language": "ts",
          "code": "const vault = container.feature('vault', { vaultId: 'notes-sync' })\nawait vault.unlock('correct horse battery staple')"
        }
      ]
    },
    "lock": {
      "description": "Drops the key from memory. A passphrase key needs `unlock()` again; a device key reloads from IndexedDB on next use.",
      "parameters": {},
      "required": [],
      "returns": "void"
    },
    "forget": {
      "description": "Deletes this vault's device key from IndexedDB, for example on sign-out. **Anything encrypted with it can no longer be decrypted.**",
      "parameters": {},
      "required": [],
      "returns": "Promise<void>"
    },
    "encrypt": {
      "description": "Encrypts a string. Creates the device key on first use, unless a `secret` was passed or `unlock()` was called.",
      "parameters": {
        "plaintext": {
          "type": "string",
    ...
    "decrypt": {
      "description": "Decrypts a payload made by `encrypt()`, by this vault or the node vault with the same key. Never creates a key.",
      "parameters": {
        "payload": {
          "type": "string",
  ...
  "getters": {
    "vaultId": {
      "description": "This vault's ID, from the `vaultId` option.",
      "returns": "string | undefined"
    },
    "fingerprint": {
      "description": "Short hash of the loaded key, recorded in every payload.",
      "returns": "string | undefined"
    },
    "isUnlocked": {
      "description": "True when a key is loaded in memory.",
      "returns": "boolean"
    }
  },
  "events": {},
