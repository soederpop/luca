 ...
 * which are commonly needed when setting up servers or network services. Use
 * `findOpenPort` before starting a server to avoid port conflicts, and
 * `isPortOpen` to test a specific port without claiming it. It also offers
 * heavier LAN tooling — host discovery, TCP port scanning, and an optional
 * nmap wrapper — for inspecting the local network.
 *
 * @example
 * ```typescript
